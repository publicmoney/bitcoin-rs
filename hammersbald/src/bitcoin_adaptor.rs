...
impl BitcoinAdaptor {
	...
	where
		T: Serialize + BitcoinHash,
	{
		Ok(self
			.hammersbald
			.put_keyed(&encodable.bitcoin_hash()[..], serde_cbor::to_vec(encodable)?.as_slice())?)
	}

	/// Retrieve a bitcoin_object with its hash
	pub fn get_hash_keyed<T>(&self, id: &sha256d::Hash) -> Result<Option<(PRef, T)>, Box<dyn Error>>
	where
		T: DeserializeOwned + BitcoinHash,
	{
...
mod test {
	...
	use super::*;
	use bitcoin::blockdata::{block::Block, constants::genesis_block, transaction::Transaction};
