...
echo "https://github.com/publicmoney/bitcoin-rs/" >> deb/DEBIAN/docs
echo "8" >> deb/DEBIAN/compat
#create control file
control=deb/DEBIAN/control
echo "Package: bitcoin-rs" >> $control
version=`grep -m 1 version Cargo.toml | awk '{print $3}' | tr -d '"' | tr -d "\n"`
echo "Version: $version" >> $control
echo "Source: bitcoin-rs" >> $control
...
echo "Vcs-Git: git://github.com/publicmoney/bitcoin-rs.git" >> $control
echo "Vcs-Browser: https://github.com/publicmoney/bitcoin-rs" >> $control
echo "Architecture: $1" >> $control
echo "Depends: libssl1.0.0 (>=1.0.0)" >> $control
echo "Description: Bitcoin network client implemented in Rust" >> $control
#build .deb package
